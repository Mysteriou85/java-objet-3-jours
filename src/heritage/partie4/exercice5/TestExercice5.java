...
public class TestExercice5 {
    public static void main(String[] args) {
        ...
        System.out.println("L'étape nommée " + testDuneEtapeSeule.getDescription()
                + " a une durée égale à " + testDuneEtapeSeule.donneDureePrevueHHMM().format(DateTimeFormatter.ofPattern("HH:mm")));
        ...
        System.out.println("La mini excursion nommée " + maMiniExc.getLibelleMiniExcursion() + " a une durée égale à "
                +maMiniExc.donneDureePrevueHHMM().format(DateTimeFormatter.ofPattern("HH:mm")));

        MiniExcursionPayante maMiniExcPay = new MiniExcursionPayante("le parcours Java extraordinaire", 15,lesEtapes,1240);
        System.out.println("La mini excursion payante nommée " + maMiniExcPay.getLibelleMiniExcursion()
                + " a une durée égale à " + maMiniExcPay.donneDureePrevueHHMM().format(DateTimeFormatter.ofPattern("HH:mm"))
        ...
        MiniExcursionPlanifiee maMiniExcPlanifB = new MiniExcursionPlanifiee("08:15:00",14,maMiniExc);
        ...
        System.out.println("Le planning de la journée : "
                + monPlanningJournee.getLaDate().format(DateTimeFormatter.ofPattern("dd/MM/yy")) + " aura pour excursion(s):");
        for(MiniExcursionPlanifiee miniExc : monPlanningJournee.getLesMiniExcursionsPlanifiees())
        {
            System.out.println(miniExc.getLaminiExcursion().getLibelleMiniExcursion() + " a une durée égale à "
                    + miniExc.getLaminiExcursion().donneDureePrevueHHMM().format(DateTimeFormatter.ofPattern("HH:mm")));
        }
    }
}
